...
function fPopCalendar(param1, param2, param3)
{
	...
	if(formatInputs==1)
	{
		...
		// william adds to avoid 'DD-MON-YY'.
		if (tmpString.match(/DD-MON-YY/i)) {
			tmpString = '';
		}
	}
	if(formatInputs==2)
	...
	//alert('datas.length: ' + datas.length + ': ' + tmpString);
	tmpo = formatType.split(formatSplitter);
	//alert('tmpo.length: ' + tmpo.length);
	dC="";tC="";
	...
	//alert('datas: ' + datas + ': ' + formatSplitter);
	createCalendarElements();
	offsets = getScrollXY();
